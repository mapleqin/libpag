...
Snapshot* RenderCache::getSnapshot(const FeatherMask* featherMask) {
  ...
  usedAssets.insert(featherMask->assetID);
  auto scaleFactor = stage->getAssetMaxScale(featherMask->assetID);
  auto snapshot = getSnapshot(featherMask->assetID);
  if (snapshot && (fabsf(snapshot->scaleFactor() - scaleFactor) > SCALE_FACTOR_PRECISION)) {
    removeSnapshot(featherMask->assetID);
    snapshot = nullptr;
  }
  if (snapshot) {
    moveSnapshotToHead(snapshot);
  ...
  snapshot->assetID = featherMask->assetID;
  snapshotCaches[featherMask->assetID] = snapshot;
  return snapshot;
}
